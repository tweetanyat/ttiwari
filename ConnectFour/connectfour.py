...
tokens=0 #variable will be used to find the longestStreak given 10 tokens to each player

nodexExpaned=0 #variable will used to calculate the number of nodes expanded by player 1
...
class ConnectFourBoard(object):
    ...
    def __init__(self, board_array = None, board_already_won = None, modified_column = None, current_player = 1, previous_move = -1,k=4):
    ...
    def set_k(self,k):
        """This Function will be used for the setting the value of k to a higher value for running the game for Q4b"""
        self.k = k
    ...
    def set_node_expanded(self):
        """ Will be used to increment the nodes expanded"""
        global nodexExpaned
        nodexExpaned = nodexExpaned + 1    
    
    def get_node_expanded(self):
        """ Will return the total number of nodes expanded"""
        global nodexExpaned
        return nodexExpaned    
    ...
    def do_move(self, column):
        ...
        if (player_id ==1): #Expand the values for player 1
            self.set_node_expanded()
        ...
        return ConnectFourBoard(new_board, board_already_won=self.is_win(), modified_column=column, current_player = self.get_other_player_id(),k=self.k)

   
        
    def _is_win_from_cell(self, row, col,k=4):
        """ Determines if there is a winning set of four connected nodes containing the specified cell """
        return ( self._max_length_from_cell(row, col) >= k )
    ...
    def _contig_vector_length_2(self, row, col, direction):
        """Returns the 2 length chains from a cell[i],[j] in a particular direction"""
        count = 0
        playerid = self.get_cell(row, col)
    ...
    def longest_chain_2(self, playerid):
        """
        Returns the total count of chains of length 2
        """        
        counter = 0
        for i in xrange(self.board_height):
            for j in xrange(self.board_width):
    ...
    def _contig_vector_length_3(self, row, col, direction):
        """Returns the 3 length chains from a cell[i],[j] in a particular direction"""
        count = 0
        playerid = self.get_cell(row, col)
    ...
    def longest_chain_3(self, playerid):
        """
        Returns the total count of chains of lenght 3
        """
        counter = 0
        for i in xrange(self.board_height):
            for j in xrange(self.board_width):
    ...
    def is_win(self):
        ...
        for i in xrange(self.board_height):
            for j in xrange(self.board_width):
                ...
                if cell_player != 0:
                    win = self._is_win_from_cell(i,j,self.k)
                    if win:
                        self._is_win = win
                        return cell_player
    ...
    def clone(self):
        ...
        return ConnectFourBoard(self._board_array, board_already_won=self._is_win, current_player = self.get_current_player_id(),k=self.k)
...
class ConnectFourRunner(object):
    ...
    * The game ends when one of the two players has four consecutive tokens in a row
    ...
    def run_game(self, verbose=True,longestStreak=0):
        ...
        while not win_for_player and not self._board.is_tie():
            #If longestStreak is set, a global variable tokens is incremented for every chance played by min or max
            if longestStreak:
                global tokens
                tokens = tokens + 1
                if tokens > 10:
                    print "Number of tokens(10) exhausted per player"
                    p1Streak = self._board.longest_chain(self._board.get_current_player_id())
                    p2Streak = self._board.longest_chain(self._board.get_other_player_id())
                    #Setting the winner of the game based on the longestSteak per player
                    if (p1Streak > p2Streak):
                        win_for_player = self._board.get_current_player_id()
                        break
                    elif (p2Streak > p1Streak):
                        win_for_player = self._board.get_other_player_id()
                        break
                    else:
                        #If the game is tie, the case is handles after the break statment by 'if win_for_player != 0 and self._board.is_tie():
                        print "Longest Streak Tie"
                        break
            else:
                pass            
            for callback, id, symbol in ( player1, player2 ):
                ...
                if self._board.is_game_over():
                    ...
                    break
        #If not playing for the game specified in Q4b, go for the normal win condition checking
        if not longestStreak:
            win_for_player = self._board.is_win()
...
def run_game(player1, player2, board = ConnectFourBoard(k=4),longestStreak=0):
    """ Run a game of Connect Four, with the two specified players """
    tic = time.clock()
    
    #Setting the value of k a higher value when running the game to find the longestStreak
    if longestStreak:
        board.set_k(k = 11)
    game = ConnectFourRunner(player1, player2, board=board)
    if longestStreak:
        #If longestStreak run set to 1 , run the game to find the longestStreak
        val = game.run_game(longestStreak=longestStreak)
    else:
        #Else run a normal game
        val = game.run_game()
        
    toc = time.clock()
    timeItr = toc - tic
    print "Execution Time: " + str(timeItr)
    print "Total Number of Nodes Expanded by Player 1: %d"%board.get_node_expanded()
    return val
